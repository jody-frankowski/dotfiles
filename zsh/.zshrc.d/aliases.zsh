...
alias glo='gi log -p'
alias glog='gi log --all --date="format:%m-%d" --graph --abbrev-commit --decorate --format=format:"%C(bold blue)%h%C(reset) %C(bold green)%ad%C(reset) %C(white)%s%C(reset) %C(bold blue)%aN%C(reset)%C(auto)%d%C(reset)"'
