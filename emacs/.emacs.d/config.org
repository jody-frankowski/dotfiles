...
Install General, which is used to set keybindings:
...
/!\ If you try to insert a ="= at the end of a line and if the next one starts with one,
=electric-pair= will consider it to be its closing twin and jump to one char after it.

#+BEGIN_SRC emacs-lisp
(electric-pair-mode)
#+END_SRC

Disable =electric-pair-mode= in command prompt (evil's ex mode):
...
(use-package evil
  ...
  :config
  ;; Evil currently has a bug where typing * or # on the first whitespace char of a line doesn't
  ;; work in some cases. See https://github.com/emacs-evil/evil/issues/1160. If you want to "fix"
  ;; this you could enable evil-symbol-word-search. But note that it would also change the word
  ;; selection of these commands. E.g. "test-123" would now be considered a word while it previously
  ;; was only "test".

  ;; Make `_` a word character as in Vim
  (modify-syntax-entry ?_ "w")

  (evil-mode 1))

;; Save the current minibuffer content to `evil-ex-history` when aborting
(defun save-evil-ex-history-on-abort (&rest _)
  (when (and (minibufferp)
             (boundp 'evil-ex-history)
             ;; Check if we are in the evil-ex command line
             (string= (minibuffer-prompt) ":"))
    (let ((cmd (minibuffer-contents)))
      (when (> (length cmd) 0)
        (add-to-history 'evil-ex-history cmd)))))
(advice-add 'abort-recursive-edit :before #'save-evil-ex-history-on-abort)
#+END_SRC
...
(use-package evil-numbers
  :config
  (csetq evil-numbers-pad-default t))
#+END_SRC

Note that we don't set =evil-search-module= to =evil-search= because it's much
slower than isearch.

Make splits (=C-w s= & =C-w v=) focus the new window below or to the right:

#+BEGIN_SRC emacs-lisp
(csetq evil-split-window-below 'true)
(csetq evil-vsplit-window-right 'true)
#+END_SRC
...
Install [[https://github.com/redguardtoo/evil-nerd-commenter][evil-nerd-commenter]] for
NERDCommenter-like keybindings:
...
Install [[https://github.com/emacs-evil/evil-surround][evil-surround]] to provide surround operators:
...
Rename magit buffers from =magit(-MODE): PROJECT= to =PROJECT magit(-MODE)=

#+BEGIN_SRC emacs-lisp
(csetq magit-buffer-name-format "%t %M")
#+END_SRC
...
Increase minibuffers inputs history. This history is shared between evil-ex-history, ivy-history
and more:
...
Decrease the headlines indentation from 2 to 1 character:

#+BEGIN_SRC emacs-lisp
(csetq org-indent-indentation-per-level 1)
#+END_SRC
...
Decode =extended keys=. This allows us, for example to distinguish =C-RET=
=(org-insert-heading-respect-content)= and =M-RET= =(org-meta-return)=. Requires Tmux's
=extended-keys on=.
https://unix.stackexchange.com/questions/79374/are-there-any-linux-terminals-which-can-handle-all-key-combinations/79561#79561
https://emacs.stackexchange.com/questions/26699/keycode-for-shift-return-on-os-x-iterm2
https://emacs.stackexchange.com/questions/1020/problems-with-keybindings-when-using-terminal/13957#13957

#+BEGIN_SRC emacs-lisp
(defun me/define-extended-key (modifier keycode event)
  "Decode an xterm modifyOtherKeys sequence as EVENT.
MODIFIER uses xterm's modifier numbering:
  2 = Shift, 3 = Meta, 5 = Control.
KEYCODE is the decimal character code used by the terminal."
  (define-key input-decode-map
              (format "\e[27;%d;%d~" modifier keycode)
              (kbd event)))

(defun me/tty-setup-extended-keys ()
  ;; Keep S-SPC equivalent to an ordinary space.  Change \"SPC\" to
  ;; \"S-SPC\" here if it should be a distinct key.
  (me/define-extended-key 2 32  "SPC")

  (me/define-extended-key 2 13  "S-<return>")
  (me/define-extended-key 3 13  "M-<return>")
  (me/define-extended-key 5 13  "C-<return>")
  (me/define-extended-key 2 127 "S-<backspace>")

  ;; Forward Delete is normally encoded as CSI 3;2~, rather than as
  ;; modifyOtherKeys character 127.
  (define-key input-decode-map
              "\e[3;2~"
              (kbd "S-<delete>")))

;; Run now for the initial terminal and again for terminals created later,
;; such as frames created by an Emacs daemon.
(me/tty-setup-extended-keys)
(add-hook 'tty-setup-hook #'me/tty-setup-extended-keys)
...
;; Make the intended Org distinction explicit.  Native terminal M-RET is
;; normally reported as M-RET, while modifyOtherKeys produces M-<return>.
...
(csetq org-todo-keywords
  '((sequence "TODO" "DOING" "WAITING" "|" "DONE" "CANCELLED" "OBSOLETE" "PAUSED")))
#+END_SRC
...
Set the outer terminal window title to the current buffer filename (or to the last one when in a
minibuffer):
...
(defun set-term-title ()
  (send-string-to-terminal
   (concat "\e]2;e "
           ...
           "\a")))
(add-hook 'post-command-hook 'set-term-title)
#+END_SRC
...
;; Set fill-column for org-mode's Emacs Lisp code blocks
(defun me/elisp-fill-column ()
  (setq-local fill-column 100))
(add-hook 'emacs-lisp-mode-hook #'me/elisp-fill-column)
#+END_SRC
...
;; Make the usual Emacs movement keybindings work in Evil Ex mode.
;; Otherwise, they would respectively, jump at the beginning of the command-line or open the Evil
;; command-line window.
